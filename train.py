...
parser = argparse.ArgumentParser(description="faceboxes Detector Training With Pytorch")
...
train_dataset = WIDERDetection(cfg.TRAIN_FILE, mode="train")
...
val_dataset = WIDERDetection(cfg.VAL_FILE, mode="val")
    ...
    for epoch in range(start_epoch, cfg.EPOCHES):
        for batch_idx, (images, targets) in enumerate(train_loader):
            ...
            if iteration % 10 == 0:
                ...
                print(
                    "->> loc loss:{:.4f} || conf loss:{:.4f}".format(
            ...
            if iteration != 0 and iteration % 5000 == 0:
                ...
                torch.save(
                    net.state_dict(),
                    os.path.join(
                        args.save_folder, "faceboxes_wider_" + repr(iteration) + ".pth"
                    ),
                )
            iteration += 1
...
def val(epoch, net, criterion):
    ...
    if tloss < min_loss:
        ...
        torch.save(net.state_dict(), os.path.join(args.save_folder, "faceboxes.pth"))
        min_loss = tloss
    ...
    torch.save(states, os.path.join(args.save_folder, "faceboxes_checkpoint.pth"))
